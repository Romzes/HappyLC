...
# Runtime = 0 ms  Beats 100.00%  ;  Memory = 17.86 MB  Beats 37.17%
class Solution:
    def compareVersion(self, version1: str, version2: str) -> int:
        parts1 = [int(s) for s in version1.split('.')]
        parts2 = [int(s) for s in version2.split('.')]
        n1, n2 = len(parts1), len(parts2)
        for i in range(max(n1, n2)):
            d1 = parts1[i] if i < n1 else 0
            d2 = parts2[i] if i < n2 else 0
            if d1 < d2: return -1
            if d1 > d2: return 1
        return 0
...
sln = Solution()
print(sln.compareVersion(version1='1.2', version2='1.10'))  # Output: -1
# Explanation: version1's second revision is "2" and version2's second revision is "10": 2 < 10, so version1 < version2.

sln = Solution()
print(sln.compareVersion(version1='1.01', version2='1.001'))  # Output: 0
# Explanation: Ignoring leading zeroes, both "01" and "001" represent the same integer "1".

sln = Solution()
print(sln.compareVersion(version1='1.0', version2='1.0.0.0'))  # Output: 0
